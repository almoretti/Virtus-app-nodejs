...
function formatAvailabilityInfo(availability) {
    if (availability.date) {
        ...
        Object.entries(availability.availability).forEach(([slot, techs]) => {
            const slotName = slotDisplayMapping[slot];
            result += `**${slotName}:**\n`;
            availability.technicians.forEach((tech) => {
                const available = techs[tech.id];
        ...
        Object.entries(availability.availability).forEach(([dateKey, dayAvailability]) => {
            ...
            result += `**${date}:**\n`;
            Object.entries(dayAvailability).forEach(([techId, slots]) => {
                const tech = availability.technicians.find((t) => t.id === techId);
                if (tech) {
                    result += `- **${tech.name}**: `;
                    const availableSlots = Object.entries(slots)
                        .filter(([_, available]) => available)
                        .map(([slot, _]) => slotDisplayMapping[slot]);
...
export async function handleToolCall(params, auth) {
...
async function checkAvailability(args, auth) {
...
async function createBooking(args, auth) {
...
async function modifyBooking(args, auth) {
...
async function cancelBooking(args, auth) {
...
async function getBookings(args, auth) {
